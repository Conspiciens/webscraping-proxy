...
def front_page_car_info(link: str, response: requests.Response) -> list[Car]: 
    ev = []
    soup = BeautifulSoup(response.html.raw_html, 'lxml')
    ...
    for car in cars: 
        car_soup = BeautifulSoup(str(car), 'lxml')

        link = car_soup.find('a', class_="title")['href']
        car_name = link.split("/")[-1]

        car_range = car_soup.find('span', class_='erange_real').get_text(strip=True)
        car_eff = car_soup.find('span', class_='efficiency')
        car_weight = car_soup.find('span', class_='weight_p')
        car_acc = car_soup.find('span', class_='acceleration_p').get_text(strip=True)
        car_kw = car_soup.find('span', class_='battery_p').get_text(strip=True)
        car_charging_speed = car_soup.find('span', class_='fastcharge_speed_print').get_text(strip=True)

        availability_str = car_soup.find('div', class_=["availability archive", "availability current"]).get_text(strip=True)

        availability = True 
        if availability_str.split(" ")[0] == "Discontinued":
            availability = False

        vehicle = Car(link, car_name, "none", 
            car_kw, car_range, "none", car_acc, availability)  
...
async def main(): 
    ...
    evs = front_page_car_info(page_list_links, response)
    print(evs)
    ...
    while page_num <= total_pages: 
        ...
        if total_pages + 1 == page_num: 
            manager.to_json_file_multi(evs, link, True)    
            page_num += 1 
            continue
        else:
            manager.to_json_file_multi(evs, link, False)
            page_num += 1
